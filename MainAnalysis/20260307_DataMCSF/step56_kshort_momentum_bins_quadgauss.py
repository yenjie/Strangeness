...
def run_one_bin(pmin: float, pmax: float, label: str):
    mc_prefix = f"step5_kshort_pbin_{label}_quadgauss_mc"
    data_prefix = f"step6_kshort_pbin_{label}_quadgauss_data"

    cmd_mc = (
        "root -l -b -q 'Code/step5_mc_random_pairs_kshort.C("
        f"\"Samples/merged_mc_v2.2.root\",\"Reports/step4_a0025_best_fit_params.csv\",\"Reports\",\"{mc_prefix}\",\"MC\","
        f"\"QuadGauss\",\"baseline\",0.40,0.70,0.40,0.70,280,{pmin:.3f},{pmax:.3f})'"
    )
    cmd_da = (
        "root -l -b -q 'Code/step5_mc_random_pairs_kshort.C("
        f"\"Samples/merged_data_v2.2.root\",\"Reports/step4_a0025_best_fit_params.csv\",\"Reports\",\"{data_prefix}\",\"Data\","
        f"\"QuadGauss\",\"baseline\",0.40,0.70,0.40,0.70,280,{pmin:.3f},{pmax:.3f})'"
    )
    run_root(cmd_mc)
    run_root(cmd_da)
...
def main():
    ...
    out_csv = REPORTS / "step56_kshort_momentum_binned_sf_quadgauss.csv"
    ...
    out_txt = REPORTS / "step56_kshort_momentum_binned_sf_quadgauss_summary.txt"
    with out_txt.open("w") as f:
        f.write("Kshort->pipi momentum-binned tagging SF (strict both-daughter p bin)\n")
        f.write("Signal model override: QuadGauss; seed: step4_a0025_best_fit_params.csv\n\n")
        for r in rows:
            f.write(
