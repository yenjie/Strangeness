...
def run_one_bin(pmin: float, pmax: float, label: str):
    mc_prefix = f"step5_kshort_pbin_{label}_mc"
    data_prefix = f"step6_kshort_pbin_{label}_data"
    ...
    cmd_mc = (
        ...
        f"\"auto\",\"baseline\",0.40,0.70,0.40,0.70,280,{pmin:.3f},{pmax:.3f})'"
    )
    cmd_da = (
        "root -l -b -q 'Code/step5_mc_random_pairs_kshort.C("
...
def main():
    # Larger bins to improve fit stability.
    bins = [
        (0.4, 1.2, "p04_12"),
        (1.2, 5.0, "p12_50"),
    ]
    ...
    out_csv = REPORTS / "step56_kshort_momentum_binned_sf.csv"
    ...
    out_txt = REPORTS / "step56_kshort_momentum_binned_sf_summary.txt"
    with out_txt.open("w") as f:
        f.write("Kshort->pipi momentum-binned tagging SF (strict both-daughter p bin)\n")
        f.write("Seed: Reports/step4_a0025_best_fit_params.csv; model=auto; bkg=baseline\n\n")
        for r in rows:
            f.write(
