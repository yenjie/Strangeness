...
def collapse_tail_1d(src, keep_bins, name):
    ...
    out = ROOT.TH1D(
        ...
        src.GetTitle(),
        keep_bins,
        src.GetXaxis().GetXmin(),
        src.GetXaxis().GetBinUpEdge(keep_bins),
    )
    out.SetDirectory(0)
    out.Sumw2()
    for ib in range(1, src.GetNbinsX() + 1):
        dest = min(ib, keep_bins)
...
def make_activity_projection(flat_hist, n_act, n_p, n_cos, name, title):
    out = ROOT.TH1D(name, title, n_act, -0.5, 60.5)
    out.SetDirectory(0)
    out.Sumw2()
    block = n_p * n_cos
...
def draw_comparison(h_nominal, h_nom_sys, h_alt, out_path, title):
    ...
    ratio = build_ratio(h_alt, h_nominal, "hYiOverNominal", ";dN_{ch}/d#eta (|#eta|<0.5);Yi / nominal")
...
def draw_closure_comparison(h_nominal_closure, h_alt_closure, out_path):
    ...
    pad2.SetTopMargin(0.03)
    pad2.SetBottomMargin(0.28)
    ratio = build_ratio(h_alt_closure, h_nominal_closure, "hYiClosureOverNominal", ";dN_{ch}/d#eta (|#eta|<0.5);Yi / nominal")
...
def main():
    if len(sys.argv) != 6:
        raise SystemExit(
            "Usage: run_yi_independent_dndeta.py "
    ...
    h_reco_counts = load_obj(f_mc, "hRecoCountsDNdEta")
    ...
    h_k_true_fine = make_activity_projection(
        h_true_gen["K"], n_act, n_p, n_cos, "hYiKTruthFine", "MC truth K;dN_{ch}/d#eta (|#eta|<0.5);Counts"
    )
    h_pi_true_fine = make_activity_projection(
        h_true_gen["Pi"], n_act, n_p, n_cos, "hYiPiTruthFine", "MC truth #pi;dN_{ch}/d#eta (|#eta|<0.5);Counts"
    )
    h_k_mc_fine = make_activity_projection(
        corrected_mc["K"], n_act, n_p, n_cos, "hYiKMcFine", "Yi-style MC K;dN_{ch}/d#eta (|#eta|<0.5);Counts"
    )
    h_pi_mc_fine = make_activity_projection(
        corrected_mc["Pi"], n_act, n_p, n_cos, "hYiPiMcFine", "Yi-style MC #pi;dN_{ch}/d#eta (|#eta|<0.5);Counts"
    )
    h_k_data_fine = make_activity_projection(
        corrected_data["K"], n_act, n_p, n_cos, "hYiKDataFine", "Yi-style data K;dN_{ch}/d#eta (|#eta|<0.5);Counts"
    )
    h_pi_data_fine = make_activity_projection(
        corrected_data["Pi"], n_act, n_p, n_cos, "hYiPiDataFine", "Yi-style data #pi;dN_{ch}/d#eta (|#eta|<0.5);Counts"
    )
    ...
    keep_bins = h_nominal.GetNbinsX()

    h_k_true = collapse_tail_1d(h_k_true_fine, keep_bins, "hYiKTruth")
    h_pi_true = collapse_tail_1d(h_pi_true_fine, keep_bins, "hYiPiTruth")
    h_k_mc = collapse_tail_1d(h_k_mc_fine, keep_bins, "hYiKMc")
    h_pi_mc = collapse_tail_1d(h_pi_mc_fine, keep_bins, "hYiPiMc")
    h_k_data = collapse_tail_1d(h_k_data_fine, keep_bins, "hYiKData")
    h_pi_data = collapse_tail_1d(h_pi_data_fine, keep_bins, "hYiPiData")
    ...
    h_double_ratio = build_ratio(
        ...
        "Yi-style double ratio;dN_{ch}/d#eta (|#eta|<0.5);(K/#pi)_{Data}/(K/#pi)_{MC}",
    )
    ...
    h_compare_ratio = draw_comparison(
        ...
        os.path.join(out_dir, "YiIndependent_DNdEta_Comparison.pdf"),
        "Yi-style independent dN_{ch}/d#eta cross-check;dN_{ch}/d#eta (|#eta|<0.5);(K/#pi)_{Data}/(K/#pi)_{MC}",
    )
    h_closure_compare_ratio = draw_closure_comparison(
        h_nominal_closure,
        h_closure,
        os.path.join(out_dir, "YiIndependent_DNdEta_ClosureComparison.pdf"),
    )
    ...
    fout = ROOT.TFile.Open(os.path.join(out_dir, "yi_independent_dndeta.root"), "RECREATE")
    ROOT.TNamed(
        "definition",
        "Independent Yi-style dN/deta chain: fake-correct observed tags in full flat mu=(dN/deta,p,|cos(theta)|), unfold observed tags in flat mu, apply truth-cell 3x3 inversion, then divide by truth-cell matched/gen efficiency.",
    ).Write()
    ...
    with open(os.path.join(out_dir, "yi_independent_dndeta.txt"), "w", encoding="ascii") as out:
    ...
    print(f"Wrote {os.path.join(out_dir, 'yi_independent_dndeta.root')}")
    print(f"Wrote {os.path.join(out_dir, 'YiIndependent_DNdEta_Comparison.pdf')}")
    print(f"Wrote {os.path.join(out_dir, 'YiIndependent_DNdEta_ClosureComparison.pdf')}")
